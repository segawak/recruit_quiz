...
QuestionLIst CreateEconomicsExam() {
	...
	for (int i = 0; i < genreCount; i++) {
		...
		for (int j = 0; j < quizCount; j++) {
			...
			//問題文のコピーを作成しつつ、選ばれなかった空欄を答えに書き換える
			...
			for (int k = 0; k < blanks.size(); k++) {
				...
				if (k != index) {
					s += question.answers[k][0];	//選ばれなかった空欄の場合は答えをコピ-
				}
				else {
					s.append("[ ? ]");	//選ばれた空欄の場合は「表示用の空欄文字列をコピー
				}
